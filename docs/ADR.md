...
- Status: Accepted, P0-P7 implemented; 0.4.18 artifact/service evidence passed
...
- Rationale: 기존 Windows service owner는 `codexwinmux.exe --codexwinmux-engine` combined engine process를 실행했다. 이 안에서 Backend API host와 Core Supervisor가 함께 실행되고, runtime worker만 child process로 분리되어 있었다. 장애 격리, 독립 restart, service upgrade, split release evidence를 요구하려면 Backend와 Core 사이에 typed command/event protocol과 별도 host process가 필요하다.
- Consequences: P1에서 `src/lib/core-engine/contracts.ts`, Core client/server adapter를 추가했고, P2에서 `codexwinmux.exe --codexwinmux-core`와 `dist/workers/core-engine-host.js` standalone Core host foundation을 추가했다. P3-P6에서 Backend API/WebSocket은 Core runtime adapter를 통과한다. `0.4.18`부터 Backend Core transport 기본값은 loopback TCP이며, UI-owned local engine과 Windows service runbook 기본값은 `codexwinmux-core`/`codexwinmux-backend` split topology다. combined Windows service start/install과 Backend in-process Core client fallback은 제거했고, 기존 combined service는 stop/uninstall migration cleanup에만 사용한다. `0.4.18` packaged artifact package gate와 실제 split service install/start/health evidence가 통과했으므로 이후 반복 수집은 release regression gate로 유지한다.

## ADR-026: Windows service account와 NSIS service option은 승격 gate 전까지 runbook-first로 유지한다

- Status: Accepted
- Decision: `0.4.18` 내부 폐쇄망 운영에서는 split Windows service를 계속 `LocalSystem + runbook-first`로 운영한다. 장기 반복 운영 목표는 전용 local Windows service account `codexwinmux-svc`이지만, profile/data dir, Codex credential/session migration, folder ACL, account rotation, upgrade/uninstall/reboot/health smoke가 닫히기 전에는 전환하지 않는다. NSIS installer에는 service install option을 기본 설치 흐름으로 넣지 않고, 같은 gate가 닫힌 뒤에만 default-off 고급 옵션으로 승격한다.
- Rationale: 현재 split service는 운영자 profile의 `HOME`, `USERPROFILE`, `LOCALAPPDATA`, Codex session/credential 위치와 강하게 연결되어 있다. 전용 계정으로 즉시 바꾸면 서비스는 더 낮은 권한 원칙에는 가까워지지만, Codex CLI 인증, session discovery, runtime storage, log/backup 위치가 모두 바뀌어 실제 내부 사용 환경을 깨뜨릴 수 있다. NSIS 자동 service 설치도 SCM mutation, upgrade, uninstall, reboot 후 복구, 계정 ACL 실패를 installer 경로에 끌어들이므로 별도 smoke 없이 기본 옵션으로 둘 수 없다.
- Consequences: `src/lib/windows-service-host.ts`의 `operationDecision`은 `serviceAccount.target=dedicated-local-service-account`, `serviceAccount.serviceAccountName=codexwinmux-svc`, `installer.nsisServiceOption=deferred-default-off`, `installer.defaultEnabled=false`를 반환한다. `src/lib/windows-service-account.ts`와 `scripts/windows-service-account.ps1`은 profile/data dir, Codex credential/session migration, ACL, password rotation, health/upgrade/uninstall/reboot readiness gate를 runbook으로 제공한다. `smoke:windows:service-host`와 `smoke:windows:service-account`는 이 결정을 release gate에서 확인한다. `scripts/windows-service.ps1`은 service profile env를 XML에 쓸 수 있고, NSIS는 account gate가 닫히기 전까지 app 설치/업데이트만 담당한다.
