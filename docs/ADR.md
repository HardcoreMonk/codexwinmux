...
## ADR-026: Windows service account와 NSIS service option은 runbook-first/default-off로 운영한다

- Status: Accepted
- Decision: `0.4.18` 내부 폐쇄망 운영에서 split Windows service의 장기 반복 운영 계정은 전용 local Windows service account `codexwinmux-svc`다. 전환은 profile/data dir, Codex credential/session migration, folder ACL, `SeServiceLogonRight`, account rotation, health/reboot-readiness evidence를 닫은 elevated runbook으로만 수행한다. NSIS installer에는 service 자동 설치를 기본 흐름으로 넣지 않고, `Windows service runbook (advanced)` default-off section만 제공한다.
...
- Consequences: `src/lib/windows-service-host.ts`의 `operationDecision`은 `serviceAccount.target=dedicated-local-service-account`, `serviceAccount.serviceAccountName=codexwinmux-svc`, `installer.nsisServiceOption=runbook-default-off`, `installer.defaultEnabled=false`를 반환한다. `src/lib/windows-service-account.ts`와 `scripts/windows-service-account.ps1`은 profile/data dir, Codex credential/session migration, ACL, `SeServiceLogonRight`, password rotation, profile-aware restart/health, upgrade/uninstall/reboot readiness gate를 runbook으로 제공한다. `smoke:windows:service-host`, `smoke:windows:service-account`, `smoke:windows:electron-packaging`은 이 결정을 release gate에서 확인한다.
