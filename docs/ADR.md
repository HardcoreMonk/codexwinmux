...
- Consequences: `src/lib/windows-service-host.ts`의 `operationDecision`은 `serviceAccount.target=dedicated-local-service-account`, `serviceAccount.serviceAccountName=codexwinmux-svc`, `installer.nsisServiceOption=deferred-default-off`, `installer.defaultEnabled=false`를 반환한다. `smoke:windows:service-host`는 이 결정을 release gate에서 확인한다. `scripts/windows-service.ps1`은 계속 canonical runbook entrypoint이며, NSIS는 app 설치/업데이트만 담당한다.
