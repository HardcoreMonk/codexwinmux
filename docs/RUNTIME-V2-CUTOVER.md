...
- 2026-05-16 Core/Backend 100% 논리 분리 기준: Phase 1 완료 기준은 Backend와 Core를 같은 local engine process 안에서 명확한 경계로 분리하는 것이다. Core는 runtime v2 workers와 terminal/storage/timeline/status source of truth를 소유하고, Backend는 HTTP/API/WebSocket adapter와 orchestration 경계로 남는다. Windows Service owner combined mode와 Core process host foundation은 추가됐지만, Backend/Core 개별 service/process split은 별도 physical separation milestone이다.
...
- Backend/Core split service의 default-on 승격. Standalone Core host와 default-off TCP attach 경로는 존재하지만 기본 process tree는 아직 Backend API host와 Core Supervisor가 같은 `--codexwinmux-engine` combined engine process 안에 있다.
