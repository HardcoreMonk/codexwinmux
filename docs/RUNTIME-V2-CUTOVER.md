...
- Status polling, JSONL watch, Web Push, session history write, dismiss/ack handling의 live ownership은 Status Worker default에 있고 Windows-local status/timeline foreground stale UI evidence는 통과했다. Public/published live target evidence 전에는 legacy JSON fallback removal을 보류한다.
...
- 이 기록은 내부 signed/local evidence다. Public SmartScreen reputation과 published/public target evidence는 외부 공개 릴리스 gate로 별도 유지한다.
...
- Public SmartScreen `passed` evidence와 published/public Phase 6 target evidence가 아직 열려 있으므로 legacy JSON fallback removal은 재개하지 않는다.
