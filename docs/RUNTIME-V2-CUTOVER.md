...
- 2026-05-04 storage backup export: `corepack pnpm runtime-v2:storage-backup`가 `workspaces.json`, `workspaces/**.json`, `runtime-v2/state.db*`를 `~/.codexwinmux/backups/runtime-v2-storage-{timestamp}/`로 복사한다. live export snapshot은 29개 파일을 복사했고 원본 삭제나 migration은 수행하지 않았다.
...
- 2026-05-04 storage write mirror/default read: `CODEXWINMUX_RUNTIME_STORAGE_V2_MODE=write|default`에서 legacy JSON workspace/layout/message-history write 직후 SQLite import mirror를 best-effort로 수행한다. `corepack pnpm smoke:runtime-v2:storage-write`는 layout write, SQLite projection, status metadata 보존을 temp HOME/DB에서 검증한다. 초기 default-read slice는 workspace/layout/message-history read가 SQLite projection을 우선 사용하고 실패 시 JSON으로 fallback하는 것을 검증했다. 2026-05-16 Core/Backend 논리 분리 slice 이후 runtime default read failure는 `RuntimeStorageUnavailableError`로 fail closed되며, JSON은 명시적 `off` rollback mirror로만 사용한다. `corepack pnpm smoke:runtime-v2:storage-default-read`는 SQLite cold read, workspace directory/sidebar/status/message-history hydration, legacy layout/updateActive write mirror 후 default read, message-history rollback JSON mirror를 temp HOME/DB에서 검증한다.
...
- 2026-05-05 lifecycle control UI/actions slice: `/experimental/runtime` 상단에 Lifecycle Control panel을 추가했다. 이 panel은 `/api/health`, `/api/v2/runtime/health`, `/api/debug/perf`를 normalize해 release metadata, terminal/storage/timeline/status mode, 24시간 observation gate, worker diagnostics, perf timing, rollback runbook을 같은 화면에 표시한다. 추가로 allowlisted action id만 받는 `/api/runtime/lifecycle/action`을 통해 `phase6-gate`, `restart-service`, `deploy-local`, `rollback-runtime-flags`를 실행할 수 있다. Restart/deploy/rollback은 exact confirmation phrase가 필요하고, audit은 `~/.codexwinmux/lifecycle-actions.jsonl`에 sanitized status event와 failure label만 남긴다. Endpoint 부분 실패는 가능한 section을 계속 렌더링하고 실패 section label만 노출하며, token/cwd/session/prompt/terminal output 원문은 표시하거나 저장하지 않는다. Rollback flag mutation과 systemd drop-in 편집은 `corepack pnpm lifecycle:rollback-dry-run`, `corepack pnpm lifecycle:rollback-apply`, Lifecycle Action `Apply Rollback Flags`로 자동화한다.
...
- 2026-05-14 Windows runtime v2 follow-up: CLI tab creation은 `CODEXWINMUX_RUNTIME_TERMINAL_V2_MODE=new-tabs`에서 Supervisor `createTerminalTab()`를 사용하도록 전환했고, storage default에서는 JSON layout append 없이 runtime storage tab을 app-facing 응답으로 반환한다. Layout tab 생성과 CLI tab 생성의 storage-default path는 직접 `layout` sync invalidation을 broadcast한다. `corepack pnpm smoke:runtime-v2:browser-sync`는 Windows checkout에서 tmux kill 없이 temp runtime v2 default server, Playwright Chromium, page-context `/api/sync` probe로 workspace mutation event와 sidebar UI 갱신을 확인했다. 이후 같은 Windows-local smoke를 workspace create/rename/group/order, layout tab create/patch/reorder/split/move/patch/close, config/keybinding refetch까지 확장했다. 브라우저 refetch는 `cache: 'no-store'`와 API `Cache-Control: no-store`로 304 캐시 경로를 차단하며, fixture는 빈 active workspace 자동 삭제와 충돌하지 않도록 v2 workspace 생성 직후 terminal tab을 함께 만든다. `corepack pnpm smoke:electron:runtime-v2`는 Windows process-tree cleanup 보정 뒤 기본 initial + 2 reconnect marker output으로 통과했다. ADB는 winget `Google.PlatformTools` 경로로 확인했고 실제 Android 기기 `R3CX10RTWFH`/SM-S928N(Android 16)에 `com.hardcoremonk.codexwinmux` `versionName=0.4.16`, `versionCode=416`이 설치된 상태를 확인했다. Windows host에 Tailscale IPv4가 없어 `adb reverse` target `http://127.0.0.1:2579`를 명시했고, `corepack pnpm smoke:android:runtime-v2`는 initial + 2 foreground `/api/v2/terminal` marker output과 blocking console/logcat 0으로 통과했다. Android smoke server는 Windows에서 `cmd.exe`/`corepack` wrapper PID가 남지 않도록 local `tsx` CLI를 `node`로 직접 실행하고 process tree를 정리한다. Timeline WebSocket server는 `terminal.get-session-info` runtime IPC를 통해 `rtv2-` Windows adapter session의 PTY PID를 조회할 수 있으며, `corepack pnpm smoke:runtime-v2:timeline-websocket-default`는 Windows runtime tab fixture, explicit JSONL subscribe, init 2 entries, append 1 entry, runtime counter init/append 1로 통과했다. `corepack pnpm smoke:runtime-v2:status-default`는 Windows runtime tab fixture에서 `/api/status/hook` permission notification, `needs-input`, ack 후 `busy` 복귀를 통과했다. `corepack pnpm smoke:runtime-v2:phase6-default-gate`는 Windows checkout에서 `node dist/server.js`로 기동한 local target `http://127.0.0.1:8121`에 대해 terminal `new-tabs`, storage/timeline/status `default`, worker diagnostics clean으로 통과했다. 같은 날 `0.4.16` Windows package를 내부 code signing certificate `CN=PureCVisor Desktop Node Internal Code Signing`으로 재패키징했고, `smoke:windows:signing-evidence`는 installer와 `win-unpacked/codexwinmux.exe` 모두 Authenticode valid, DigiCert RFC3161 timestamp present, internal SmartScreen scope accepted로 통과했다. 서명된 산출물 기준 `corepack pnpm smoke:windows:package-gate`도 zip/update metadata/local updater/packaged launch/engine lifecycle/packaged runtime v2/installer runtime v2를 모두 통과했다. Public SmartScreen launch evidence와 published/public Phase 6 target evidence는 외부 공개 배포 gate로 별도 유지한다. 내부 폐쇄망 범위에서는 이 signed/local package evidence와 폐쇄망 Phase 6 target evidence를 legacy JSON fallback removal gate 근거로 사용할 수 있다.
- 2026-05-15 status/timeline stale UI evidence: `corepack pnpm smoke:runtime-v2:status-timeline-stale-ui`가 Windows checkout에서 temp runtime v2 default server와 Playwright Chromium으로 통과했다. 이 smoke는 session list에서 local `jsonlPath`를 노출해 resume fallback 없이 timeline을 열고, explicit timeline subscribe 뒤 `agentJsonlPath`를 runtime storage metadata에 저장하며, native background event에서 status/timeline WebSocket을 닫은 뒤 background JSONL append와 `/api/status/hook` permission prompt를 foreground reconnect 후 UI에 반영하는지 확인한다. 통과 evidence는 `statusV2Mode=default`, `timelineV2Mode=default`, status WebSocket construct/open/close `2/2/1`, timeline construct/open/close `3/2/2`, foreground timeline append 표시, status `입력 대기` 표시다. 같은 날 `v0.4.15` public installer URL과 SHA-256 `68ea233834ce254064545b2194a6844d6c7fb7051f65367dccf917de11042480`로 `corepack pnpm smoke:windows:smartscreen-public-evidence`를 재실행했지만 Chromium download, SHA match, Internet ZoneId=3까지만 통과하고 launch evidence가 Windows `Start-Process` 취소/SmartScreen reputation 단계에서 실패했다. 이 public 실패는 외부 공개 배포 전용 blocker이며, 내부 폐쇄망 릴리스와 내부 legacy JSON fallback 제거 gate에는 적용하지 않는다.
- 2026-05-16 내부 폐쇄망 release scope: codexwinmux 현재 배포 범위는 내부 폐쇄망 전용이다. 내부 gate는 signed/timestamped Windows artifact, `internal-not-required` 또는 `internal-trusted-root` SmartScreen scope, Windows local/폐쇄망 Phase 6 target smoke, status/timeline stale UI smoke, Android LAN smoke를 기준으로 판단한다. Public SmartScreen reputation과 published/public Phase 6 target evidence는 외부 공개 배포를 시작할 때만 release blocker로 승격한다.
- 2026-05-16 Core/Backend 100% 논리 분리 기준: Phase 1 완료 기준은 Backend와 Core를 같은 local engine process 안에서 명확한 경계로 분리하는 것이다. Core는 runtime v2 workers와 terminal/storage/timeline/status source of truth를 소유하고, Backend는 HTTP/API/WebSocket adapter와 orchestration 경계로 남는다. 이후 `0.4.18` source cutover에서 Backend/Core 개별 service/process split을 기본 topology로 승격했다.
- 2026-05-16 Core/Backend 논리 분리 완료 evidence: runtime storage default read failure는 `RuntimeStorageUnavailableError`로 fail closed하며 workspace/layout/message-history JSON read fallback을 사용하지 않는다. `corepack pnpm smoke:runtime-v2:storage-default-read`, `smoke:runtime-v2:status-default`, `smoke:runtime-v2:timeline-websocket-default`, `smoke:runtime-v2:status-timeline-stale-ui`, local `smoke:runtime-v2:phase6-default-gate`, `smoke:android:runtime-v2`, `smoke:windows:signing-evidence`, `smoke:windows:package-gate`, `tsc --noEmit`, `lint`, 전체 `test`가 통과했다. 세부 handoff는 `docs/operations/2026-05-16-core-backend-logical-separation-100.md`에 기록한다.
- 2026-05-16 Core process host P2 evidence: `src/lib/core-engine/contracts.ts`, `client.ts`, `server.ts`로 Backend/Core typed protocol을 고정했고, `codexwinmux.exe --codexwinmux-core`, `electron/core-process.ts`, `src/lib/core-engine/process-host.ts`, `src/workers/core-engine-host.ts`를 추가했다. `corepack pnpm test tests/unit/lib/core-engine/contracts.test.ts tests/unit/electron/engine-process.test.ts tests/unit/lib/core-engine/process-host.test.ts`, `corepack pnpm tsc --noEmit`, `corepack pnpm lint`, `corepack pnpm build:server`, `corepack pnpm build:electron:main`이 통과했다. 이 P2 slice 당시 운영 기본값은 combined engine mode였고, Backend API/WebSocket Core client 전환은 P3 범위였다.
- 2026-05-16 Core/Backend P3-P6 external attach evidence: Backend API/WebSocket은 `core-engine/runtime-api` adapter를 통과하고, Core command surface는 workspace/tab mutation, timeline read/message counts, status live command까지 포함한다. `CODEXWINMUX_CORE_ENGINE_TRANSPORT=tcp`를 지정하면 Backend adapter가 독립 Core process에 loopback TCP로 attach하며, `smoke:windows:core-backend-external-transport`가 이 경로의 `core.health` evidence를 수집한다. Release/package gate에는 standalone Core IPC, Backend external Core attach, split lifecycle dry-run이 포함된다. 이 P3-P6 slice 당시 운영 기본값은 아직 combined engine mode였다.
- 2026-05-16 Windows 0.4.17 packaged install evidence: `release/codexwinmux-Setup-0.4.17.exe`를 기본 사용자 설치 경로 `C:\Users\yohan\AppData\Local\Programs\codexwinmux`에 silent install했고, 설치된 `codexwinmux.exe`를 대상으로 `CODEXMUX_WINDOWS_PACKAGED_RUNTIME_V2=1 node scripts/smoke-windows-packaged-launch.mjs`가 packaged launch, local server health, Electron cookie auth, runtime v2 workspace/tab 생성, terminal WebSocket attach, workspace cleanup, console clean을 통과했다. `post-build --electron`은 standalone server가 직접 require하는 `nanoid`와 `zod`를 `.next/standalone/node_modules`에 보강하며, zip smoke는 `zip-entry-standalone-runtime-deps`로 이를 검증한다.
- 2026-05-16 split service stability evidence: `CODEXWINMUX_WINDOWS_SPLIT_LIFECYCLE_MUTATE=1`와 `CODEXWINMUX_WINDOWS_SPLIT_LIFECYCLE_STABILITY_MS=180000`으로 `node scripts/smoke-windows-core-backend-split-lifecycle.mjs`를 실행했다. Smoke는 `codexwinmux-core`와 `codexwinmux-backend`를 실제 Windows service로 설치/시작하고, backend restart와 core restart 후 180초 stability hold 동안 backend health sample 33개를 통과한 뒤 backend/core 순서로 cleanup했다. 이후 검증용 split service는 제거했고 기본 `codexwinmux` combined service config를 다시 썼다.
- 2026-05-16 Windows 0.4.18 split default-on cutover: `package.json` version을 `0.4.18`로 올렸고 Backend Core transport 기본값을 TCP로 승격했다. Electron UI-owned local engine은 `--codexwinmux-core`와 `--codexwinmux-engine` paired process를 시작하며, Windows service helper 기본 `-Mode`는 `split`이다. Backend `runtime-api`는 더 이상 runtime Supervisor singleton을 직접 import하거나 in-process Core client를 생성하지 않는다. Core process host는 terminal stdout/close event를 TCP transport로 forward한다. focused evidence는 `corepack pnpm exec vitest run tests/unit/lib/core-engine/transport-config.test.ts tests/unit/lib/windows-service-host.test.ts tests/unit/electron/engine-process.test.ts tests/unit/pages/runtime-direct-import-policy.test.ts tests/unit/scripts/windows-core-backend-split-lifecycle-lib.test.ts tests/unit/scripts/windows-package-gate-lib.test.ts tests/unit/scripts/windows-release-gate-lib.test.ts`, `corepack pnpm exec tsx scripts/smoke-windows-service-host.ts`, `corepack pnpm exec tsc --noEmit` 통과다. 전체 regression도 `corepack pnpm test` 182 files/865 tests passed, `corepack pnpm lint` 통과다. Windows `release/` artifact는 `corepack pnpm pack:electron`으로 재생성했고 `node scripts/smoke-windows-package-gate.mjs`가 zip, update metadata, updater local feed, packaged launch, engine lifecycle, standalone Core IPC, Backend external Core attach, split lifecycle dry-run, packaged runtime v2, installer runtime v2를 통과했다. 실제 Windows service도 old combined `codexwinmux`를 uninstall한 뒤 기본 split mode로 `codexwinmux-core`와 `codexwinmux-backend`를 install/start했고 `/api/health`는 `version=0.4.18`을 반환했다.
...
- storage `off` rollback JSON stores, config/keybinding stores. Runtime default의 workspace/layout/message-history JSON read fallback은 2026-05-16 slice에서 제거했고 read/write failure는 typed error로 fail closed한다.
...
- Current backup slice: `corepack pnpm runtime-v2:storage-backup` copies legacy JSON stores and `runtime-v2/state.db*` into `~/.codexwinmux/backups/runtime-v2-storage-{timestamp}/`.
...
- Current default-read slice: `CODEXWINMUX_RUNTIME_STORAGE_V2_MODE=default` routes workspace/layout/message-history reads through SQLite projection, keeps rollback JSON mirrors, and fails closed with `RuntimeStorageUnavailableError` if SQLite projection is unavailable. This slice is verified by `corepack pnpm smoke:runtime-v2:storage-default-read` on temp HOME/DB.
...
- 2026-05-05 WebSocket default ownership slice: 기존 `/api/timeline` WebSocket URL은 유지하되 default mode의 init/append/error/session-changed delivery는 `src/lib/runtime/timeline-ws.ts`와 `src/lib/runtime/timeline-runtime-adapter.ts`를 통해 Timeline Worker로 위임한다. Legacy mode는 `CODEXWINMUX_RUNTIME_TIMELINE_V2_MODE=off`에서 유지한다.
...
- `corepack pnpm smoke:runtime-v2:status-timeline-stale-ui` passes and proves a backgrounded status hook permission prompt refreshes to `입력 대기` after foreground reconnect.
...
- Keep a documented explicit rollback path for at least one release. Runtime default must not silently read legacy JSON fallback.
...
- Do not delete `~/.codexwinmux/runtime-v2/state.db` during first rollback; keep it for recovery and diagnostics.
...
2026-05-14 Windows 서명 로컬 종료 기준:

- `corepack pnpm pack:electron`을 `CODEXWINMUX_WINDOWS_CERTIFICATE_SHA1=8C5F3B5030D3A54B1150C2C30CFD9868800DF0C6`, publisher `PureCVisor Desktop Node Internal Code Signing`, DigiCert timestamp server로 다시 실행했다.
- `corepack pnpm smoke:windows:signing-evidence`는 `codexwinmux-Setup-0.4.16.exe`와 `release/win-unpacked/codexwinmux.exe`에 대해 Authenticode `Valid`, timestamp present, internal SmartScreen scope accepted로 통과했다.
- `corepack pnpm smoke:windows:package-gate`는 서명된 `0.4.16` 산출물 기준 updater local feed, engine lifecycle, packaged runtime v2, installer runtime v2를 포함해 통과했다.
- `corepack pnpm smoke:runtime-v2:phase6-default-gate`는 Windows local `node dist/server.js` target `http://127.0.0.1:8121`에서 terminal `new-tabs`, storage/timeline/status `default`, worker counter clean으로 통과했다.
- 이 기록은 내부 signed/local evidence다. 내부 폐쇄망 릴리스와 내부 fallback removal gate는 이 계열의 signed/local/폐쇄망 target evidence로 판단한다. Public SmartScreen reputation과 published/public target evidence는 외부 공개 릴리스 gate로 별도 유지한다.

2026-05-15 Windows public/stale UI gate follow-up:

- `corepack pnpm smoke:runtime-v2:status-timeline-stale-ui`는 temp Windows runtime v2 default server에서 통과했다. Session index local `jsonlPath` 노출, explicit timeline subscribe 뒤 `agentJsonlPath` metadata persistence, native background socket close, background JSONL append/status hook mutation, foreground reconnect UI refresh를 확인했다.
...
corepack pnpm smoke:runtime-v2:status-timeline-stale-ui
