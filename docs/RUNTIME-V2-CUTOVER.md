...
- 2026-05-16 Core/Backend 100% 논리 분리 기준: 이번 완료 목표는 Backend와 Core를 같은 local engine process 안에서 명확한 경계로 분리하는 Phase 1이다. Core는 runtime v2 workers와 terminal/storage/timeline/status source of truth를 소유하고, Backend는 HTTP/API/WebSocket adapter와 orchestration 경계로 남는다. 별도 Windows Service owner 또는 Core/Backend 개별 서비스 프로세스는 Phase 2 후속 범위다.
...
- Status polling, JSONL watch, Web Push, session history write, dismiss/ack handling의 live ownership은 Status Worker default에 있고 Windows-local status/timeline foreground stale UI evidence는 통과했다. 내부 폐쇄망 릴리스에서는 public/published evidence를 기다리지 않고, 내부 Phase 6 target과 signed/local package evidence 기준으로 legacy JSON fallback removal을 재개할 수 있다.
