...
- Windows package/update smoke: `pack:electron`, `smoke:windows:updater-local-feed`, `smoke:windows:package-gate`, `smoke:windows:release-gate`가 Windows host에서 통과했다. Local feed smoke는 기존 installer `0.4.2`에서 synthetic `0.4.3` feed를 받아 download, `quitAndInstall`, post-update launch, uninstall cleanup까지 확인한다. Published channel smoke는 `HardcoreMonk/codexwinmux` `v0.4.8` release asset 기준 `0.4.2 -> 0.4.8` channel evidence를 확인했다. Full GitHub feed smoke는 실제 `0.4.2` installer를 temp install path에 설치한 뒤 GitHub-hosted `0.4.8`로 download/install/`quitAndInstall`/post-update runtime v2 terminal/5분 long-run health까지 통과했다.
