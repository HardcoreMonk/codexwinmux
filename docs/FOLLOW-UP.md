...
- Core/Backend adapter follow-up: `layout-store`, `workspace-store`, status/timeline/cleanup Backend shared module의 runtime default/shadow 경로를 Core runtime API로 전환했다. Workspace create/delete/rename/group/order, layout pane/tab mutation, tab status metadata read/write, status live client/sync/subscribe, session history, Web Push, terminal session info가 typed Core command를 통과하며, direct import policy test가 API route와 Backend shared module의 runtime Supervisor 직접 import를 차단한다.
- Runtime timeline adapter follow-up: `timeline-ws`와 `timeline-live-shadow`의 direct Supervisor fallback을 제거하고 `timeline-runtime-adapter`로 분리했다. direct import policy test는 Backend shared module뿐 아니라 runtime timeline surface도 Supervisor를 직접 import하지 못하게 검증한다.
...
| GitHub release / published channel | 통과 | `v0.4.19` tag를 `57d49daf`에 생성/푸시하고 GitHub Release `https://github.com/HardcoreMonk/codexwinmux/releases/tag/v0.4.19`를 발행했다. Assets는 `latest.yml`, `codexwinmux-Setup-0.4.19.exe`, matching `.blockmap`, `codexwinmux-0.4.19-win.zip`이다. `CODEXMUX_WINDOWS_UPDATER_CURRENT_VERSION=0.4.18 corepack pnpm smoke:windows:updater-published-channel`은 `latestVersion=0.4.19`, installer size/sha512/blockmap/download URL checks로 통과했다 |
