...
| Legacy JSON fallback removal | 보류 유지 | status/timeline stale UI local evidence는 닫혔지만 public SmartScreen `passed` evidence와 published/public Phase 6 target evidence가 아직 열려 있으므로 fallback removal은 재개하지 않는다 |
